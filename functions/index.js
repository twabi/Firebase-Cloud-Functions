...
exports.getQuotation = functions.https.onRequest(async (request, response) => {
    ...
    if (!request.body.products || !request.body.date || !request.body.customer || !request.body.quotationNumber) {
        response.status(400).send("Missing required data: products, date, customer, or quoteNumber");
        return;
    }
    ...
    let fileName = ''
    quoteType.toLowerCase() === "invoice" ? fileName = "INVOICE" : fileName = "QUOTATION"

    try {
        const templateBuffer = await new Promise((resolve, reject) => {
            readFile(join(__dirname, "templates", `${fileName}.xlsx`), (err, data) => {
                if (err) reject(err);
                else resolve(data);
            });
        }); // Use promises for async file reading

        const template = new XlsxTemplate(templateBuffer); // Use the Buffer directly
        ...
        const values = {
            ...
            item: productList.map((product, index) => ({ // Map products for the template
                no: index + 1,
        ...
        // Generate Excel File
        const data = template.generate({type: "nodebuffer"}); // Get a Node.js Buffer
        console.log(data)
        response.setHeader(
            "Content-Type",
